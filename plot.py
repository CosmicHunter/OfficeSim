...
def main():
    ...
    parser.add_argument('--plot_g1' , default = False , action = 'store_true')
    parser.add_argument('--window_size', type=int, default=20)
    args = parser.parse_args()

    # define the names of the models you want to plot and the longest episodes you want to show
    models = ['All-Reward Model','LSTM-RL', 'SARL', 'OM-SARL']
    max_episodes = 25000
    ...
    for i, log_file in enumerate(args.log_files):
        ...
        train_pattern = r"TRAIN in episode (?P<episode>\d+) has success rate: (?P<sr>[0-5].\d+), " \
                        r"collision rate: (?P<cr>[0-5].\d+), nav time: (?P<time>\d+.\d+), " \
                        r"total reward: (?P<reward>[-+]?\d+.\d+)"
        ...
        for ep in train_episode:
            print("Episode : " , ep)
        train_reward_grp1 = []
        train_pattern_grp1 = r"total reward for group 1 agent is (?P<reward2>[-+]?\d+.\d+)"
        # print(len(train_reward))
        # print(len(train_episode))
        
        for r  in re.findall(train_pattern_grp1 , log):
            train_reward_grp1.append(float(r))
        # print(len(re.findall(train_pattern_grp1,log)))
        ...
        train_reward_grp1 = train_reward_grp1[:max_episodes]
        ...
        train_reward_grp1_smooth = running_mean(train_reward_grp1 , args.window_size)
        # plot sr
        ...
        if args.plot_g1:
            if ax1 is None:
                _, ax1 = plt.subplots()
            if args.plot_train:
                ax1.plot(range(len(train_reward_grp1_smooth)), train_reward_grp1_smooth)
                ax1_legends.append(models[i])
           

            ax1.legend(ax1_legends)
            ax1.set_xlabel('Episodes')
            ax1.set_ylabel('Group 1 Agent Rewards')
            ax1.set_title('Cumulative Discounted Rewards Group-1 Agent')
        ...
        if args.plot_sr:
            ...
            ax1.set_title('Combined Success rate for all agents [0-5]')
        ...
        if args.plot_time:
            ...
            ax2.set_title("Navigation Completion Time For Entire Simulation")
        ...
        if args.plot_cr:
            ...
            ax3.set_title('Collision Rate [0-1]')
        ...
        if args.plot_reward:
            ...
            ax4.set_title('Average Cumulative Discounted Reward Per Group-0 Agent')
